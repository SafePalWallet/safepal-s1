...
void settings_set_default() {
    ...
    gSettings->mBtcReceiveAddrType = 2;//Native Segwit
...
static int *findKey2Value(const char *key) {
	...
	do {
        ...
        CHECK_KEY(SETTING_KEY_BTC_RECV_ADDRESS_TYPE, gSettings->mBtcReceiveAddrType)
...
int settings_backup_to_nvm(int force) {
    ...
    info.mBtcMultiAddress = gSettings->mBtcMultiAddress;
    info.mBtcReceiveAddrType = gSettings->mBtcReceiveAddrType;
...
static int settings_read_from_nvm() {
    ...
    gSettings->mBtcMultiAddress = info.mBtcMultiAddress;
    gSettings->mBtcReceiveAddrType = info.mBtcReceiveAddrType;
...
int settings_init() {
	...
	gSettings = (SettingsInfo *) calloc(1, sizeof(SettingsInfo));
	if (!gSettings) {
		db_error("init settings false");
		return -1;
	}
	gSettings->mLang = CONFIG_DEFAULT_LANG;
	settings_set_default();
	int have_ota_ok = access(OTA_SUCCESS_FILE, F_OK) ? 0 : 1;
	db_msg("loading %s have otaok:%d", WALLET_SETTINGS_FILE, have_ota_ok);
	if (access(WALLET_SETTINGS_FILE, F_OK) == 0) {
		config_file_read(WALLET_SETTINGS_FILE, _read_callback, 0);
	} else {
		need_read_nvm++;
	}
	if (!gHaveSeed) {
	...
	if (have_ota_ok && !gSettings->mOtaPreVersion) {
		need_read_nvm++;
	}
    ...
    if (gSettings->mOtaPreVersion > 10000 && gSettings->mOtaPreVersion < 10059) {
        settings_save(SETTING_KEY_BTC_MULTI_ADDRESS, 1);
    }

    if (gSettings->mOtaPreVersion > 10000 && gSettings->mOtaPreVersion < 10069) {
        settings_save(SETTING_KEY_BTC_RECV_ADDRESS_TYPE, 0);
    }

	if (gSettings->mOtaPreVersion > 10012 && !have_ota_ok) {
		db_msg("OTA not success?");
		gSettings->mOtaPreVersion = 0;
	}
	if (!gHaveSeed) {
    ...
    if (device_can_brightness_adjust() == 0) {
        if (gSettings->mBrightness <= 0) {
            gSettings->mBrightness = 3;
        }
        device_set_brightness_level(gSettings->mBrightness);
    }

    if (gSettings->mRandPinKeypad > 1) {
        gSettings->mRandPinKeypad = 1;//OFF
    }
...
int settings_save(const char *key, int val) {
	int ret = config_file_set_int(WALLET_SETTINGS_FILE, key, val);
	if (ret == 0) {
		int *target = findKey2Value(key);
		if (target != NULL) {
			*target = val;
...
int settings_save_string(const char *key, const char *value) {
	...
	if (havespace) {
		...
		ret = config_file_set(WALLET_SETTINGS_FILE, key, tmpbuf);
	} else {
		ret = config_file_set(WALLET_SETTINGS_FILE, key, value);
	}
	if (ret == 0 && upatelocal) {
...
int settings_get_string(const char *key, char *value, int size) {
	return config_file_get(WALLET_SETTINGS_FILE, key, value, size);
}

int settings_get(const char *key, int default_val) {
	return config_file_get_int(WALLET_SETTINGS_FILE, key, default_val);
}

int settings_have_set(const char *key) {
	char value[32];
	return config_file_get(WALLET_SETTINGS_FILE, key, value, 32) > 0;
}
