...
int wallet_initDefaultCoin(const unsigned char *passwd) {
    ...
    wallet_genDefaultPubHDNode(passwd, COIN_TYPE_BITCOIN, COIN_UNAME_BTC4); //BTC bip86
...
int wallet_getHDNode(int type, const char *uname, HDNode *hdnode) {
	...
	db_secure("curve:%p depth:%d child_num:0x%x chain_code:%s", hdnode->curve, hdnode->depth, hdnode->child_num, debug_ubin_to_hex(hdnode->chain_code, 32));
...
int wallet_verify_seed_xpub(const unsigned char *seed, int seed_len) {
	...
	for (i = 0; i < total; i++) {
		...
		if (pubnode.depth != node.depth || pubnode.child_num != node.child_num) {
			db_error("invalid pubnode depth:%d child_num:%d != node depth:%d child_num:%d", pubnode.depth, pubnode.child_num, node.depth, node.child_num);
			ret = -101;
			break;
		}
