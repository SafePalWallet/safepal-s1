...
static int on_sign_show(void *session, DynamicViewCtx *view) {
    ...
    if (is_not_empty_string(msg->app_name)) {
    ...
    db_msg("msg->msg_type:%d, msg->bin_msg.size:%d", msg->msg_type, msg->bin_msg.size);
    if (msg->msg_type == CUST_MSG_TYPE_TXT) {
        int offset = -27;
        ...
        if (msg->bin_msg.size > 0) {
            ...
            db_msg("msg_len:%d", strlen(msg->msg));
            db_msg("msg->msg:%s", msg->msg);
            ...
            if (strlen(msg->msg) > 100) {
                ...
                int newlines = count_newlines(msg->msg);
                db_msg("newlines:%d, msg len:%d", newlines, strlen(msg->msg));
                if (newlines > 1 && strlen(msg->msg) > 26) {
                    view->total_height = SCREEN_HEIGHT * 2;
                    view_add_txt(TXS_LABEL_SIGN_MSG2, msg->msg);
                } else {
                    memzero(tmpbuf, sizeof(tmpbuf));
                    omit_string(tmpbuf, msg->msg, 40, 30);
                    view_add_txt(TXS_LABEL_SIGN_MSG, tmpbuf);
                }
            }
        }
    } else if ((msg->msg_type == CUST_MSG_TYPE_LOGIN) || (msg->msg_type == CUST_MSG_TYPE_AUTH)) {
        snprintf(tmpbuf, sizeof(tmpbuf), "%s:", res_getLabel(LANG_LABEL_WALLET_NAME));
        view_add_txt(TXS_LABEL_TITLE1, tmpbuf);
        if (get_device_name(tmpbuf, sizeof(tmpbuf), 1) > 0) {
        ...
        HWND h = view_add_txt(TXS_LABEL_VALUE2, msg->withdraw.addrss);
        int off = 0;
        if (!strcmp(msg->withdraw.coin_name, "XLM")) {
            SetWindowFont(h, res_getFont(5)); //font_14
            off = 15;
        }
        ...
        db_error("invalid msg_type:%d", msg->msg_type);
