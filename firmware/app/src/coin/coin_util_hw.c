...
int is_coin_address_long(int coinid, const char *uname) {
    if (coinid == COIN_TYPE_BITCOIN && strncmp(uname, COIN_UNAME_BTC4, 5) == 0) {
		return 1;
	}
	
    return (coinid == COIN_TYPE_EOS ||
            coinid == COIN_TYPE_XLM ||
            coinid == COIN_TYPE_NEAR ||
            coinid == COIN_TYPE_APTOS_TEST ||
            coinid == COIN_TYPE_APTOS ||
			coinid == COIN_TYPE_SUI ||
			coinid == COIN_TYPE_SUI_TEST || 
			coinid == COIN_TYPE_CARDANO ||
			coinid == COIN_TYPE_BITCOIN_TEST ||
			coinid == COIN_TYPE_EGLD ||
			coinid == COIN_TYPE_KASPA ||
			coinid == COIN_TYPE_VENOM ||
			coinid == COIN_TYPE_CHR
			) ? 1 : 0;
}
