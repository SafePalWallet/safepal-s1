...
#include "coin/Venom/venom_sign.c"
...
static int on_sign_show(void *session, DynamicViewCtx *view) {
    ...
    if ((char) msg->operation_type == VENOM_TRANSFER || (char) msg->operation_type == VENOM_TOKEN_TRANSFER) {
        if (strcmp(coin_uname, "VENOM") == 0) {
    ...
    if ((char) msg->operation_type == VENOM_TRANSFER || (char) msg->operation_type == VENOM_TOKEN_TRANSFER) {
        ...
        db_msg("venom send_value:%.8lf", send_value);
        ...
        snprintf(tmpbuf, sizeof(tmpbuf), "%s VENOM", tmpbuf);
