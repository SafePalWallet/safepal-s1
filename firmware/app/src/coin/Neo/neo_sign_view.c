...
static int show_invocation_tx(coin_state *s, DynamicViewCtx *view, struct pbc_rmessage *rmsg) {
	...
	snprintf(buff, sizeof(buff), "≈%s%.2f", money_symbol, ex_rate * send_value);
	view_add_txt(TXS_LABEL_TOTAL_MONEY, buff);
...
static int show_contract_tx(coin_state *s, DynamicViewCtx *view, InOutPuts *inouts) {
	...
	for (int i = 0; i < inouts->output_n; i++) {
		...
		if (is_empty_string(inouts->outputs[i].address)) {
			dwin_add_txt_offset(view, MK_txs_dlabel_payto_address, viewid++, s->myaddr, offset);
		} else {
			dwin_add_txt_offset(view, MK_txs_dlabel_payto_address, viewid++, inouts->outputs[i].address, offset);
		}
